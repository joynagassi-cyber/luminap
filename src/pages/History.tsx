...
import { useTransactions, useAccounts, useOrgUnits, useEvents, useCategories, useAuditEntries } from '@/lib/dataLayer';
...
export default function HistoryPage() {
  ...
  const { accounts: idbAccounts, orgUnits: idbOrgUnits, events: idbEvents, categories: idbCategories, auditEntries: idbAudit, isLoading } = useLocalStore();
  ...
  const pendingAmount = filteredTransactions.filter((t: any) => t.status === 'PENDING').reduce((s: number, t: any) => s + t.amount, 0);
  ...
  const monthlyData = MONTHS_FR.map((month, idx) => {
    ...
    const income = monthTxs.filter((t: any) => t.type === 'INCOME' && t.status === 'APPROVED').reduce((s: number, t: any) => s + t.amount, 0);
    const expense = monthTxs.filter((t: any) => t.type === 'EXPENSE' && t.status === 'APPROVED').reduce((s: number, t: any) => s + t.amount, 0);
      ...
      <div className="max-w-lg mx-auto px-5 pb-32 pt-16">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="rounded-xl p-4 text-center" style={{ backgroundColor: '#212121' }}>
            ...
            <p className="text-[#1DB954] font-bold text-base">+{formatCentsToFCFA(totalIncome)}</p>
          </div>
          <div className="rounded-xl p-4 text-center" style={{ backgroundColor: '#212121' }}>
            <p className="text-text-tertiary text-xs mb-1">Sorties</p>
            <p className="text-[#E51332] font-bold text-base">-{formatCentsToFCFA(totalExpense)}</p>
          </div>
          <div className="rounded-xl p-4 text-center" style={{ backgroundColor: '#212121' }}>
            <p className="text-text-tertiary text-xs mb-1">Résultat</p>
            <p className="font-bold text-base" style={{ color: totalIncome - totalExpense >= 0 ? '#1DB954' : '#E51332' }}>
              {totalIncome - totalExpense >= 0 ? '+' : '-'}{formatCentsToFCFA(Math.abs(totalIncome - totalExpense))}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto pb-2">
          {TABS.map(tab => (
            <button
              ...
              style={{
                backgroundColor: activeTab === tab.key ? '#FF6B00' : '#212121',
        ...
        <div className="rounded-xl p-4 mb-5" style={{ backgroundColor: '#212121' }}>
          <ChartContainer config={{}} className="h-48">
            <AreaChart data={monthlyData}>
              ...
              <Area type="monotone" dataKey="income" stackId="1" stroke={COLORS.income} fill={COLORS.income} fillOpacity={0.3} />
              <Area type="monotone" dataKey="expense" stackId="1" stroke={COLORS.expense} fill={COLORS.expense} fillOpacity={0.3} />
            </AreaChart>
          </ChartContainer>
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <p className="text-text-primary font-semibold text-sm">Activité récente</p>
          <button onClick={() => navigate('/trace')} className="text-xs" style={{ color: '#FF6B00' }}>
            Voir tout →
          </button>
        </div>
        <div className="space-y-2 mb-6">
          {filteredTransactions.slice(0, 5).map((tx: any) => (
            ...
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: tx.type === 'INCOME' ? '#1DB95420' : '#E5133220' }}>
                {tx.type === 'INCOME' ? <ArrowUpRight className="w-5 h-5" style={{ color: '#1DB954' }} /> : <ArrowDownRight className="w-5 h-5" style={{ color: '#E51332' }} />}
              </div>
              ...
              <span className="font-bold text-sm" style={{ color: tx.type === 'INCOME' ? '#1DB954' : '#E51332' }}>
                {tx.type === 'INCOME' ? '+' : '-'}{formatCentsToFCFA(tx.amount)}
              </span>
            </button>
          ))}
        </div>
