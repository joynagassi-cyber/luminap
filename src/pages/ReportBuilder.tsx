...
export default function ReportBuilder() {
  ...
  const addMetric = (field: string, fn: string, alias: string) => {
    setReport((prev) => ({
      ...prev,
      metrics: [...(prev.metrics || []), `${field}:${fn}:${alias || field}`],
    }));
  };
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>ReportBuilder</IonTitle>
        </IonToolbar>
      </IonHeader>
        ...
        <div className="min-h-screen bg-canvas">
          ...
          <div className="max-w-lg mx-auto px-5 pb-32 pt-16">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-text-tertiary text-xs mb-1.5 block">
                  Dimensions (groupement)
                </label>
                <div className="flex flex-wrap gap-2 mb-2">
                  {(
                    ["month", "year", "sourceCaisseId", "categoryId"] as const
                  ).map((dim) => (
                    <button
                      key={dim}
                      ...
                      style={
                        report.dimensions?.includes(dim)
                          ? {
            ...
            {showPreview && previewResult && (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-text-tertiary text-xs">
                    {previewResult.total} ligne
                    {previewResult.total !== 1 ? "s" : ""}
                  </span>
                </div>
                    ...
                    <thead>
                      <tr>
                        {previewResult.columns.map((col) => (
                          <th
                            key={col}
                            className="text-left py-1.5 px-2 text-text-tertiary font-medium"
                          >
                            {col}
                          </th>
                        ))}
                        ...
                        .map((row: any, i: number) => (
                          ...
                          >
                            {previewResult.columns.map((col) => (
                              <td
                                key={col}
                                className="py-1.5 px-2 text-text-primary"
                              >
                                {row[col]}
                              </td>
                            ))}
