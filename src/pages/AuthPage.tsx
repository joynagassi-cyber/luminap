...
export default function AuthPage() {
    ...
    const handleCallback = async () => {
      if (location.pathname === '/auth/callback') {
        ...
        if (result.error) {
          ...
          await oneSignalService.login(result.profile.role as any, result.profile.id);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const result = await authService.signUpWithEmail(email, password, firstName, lastName, role as any);
    if (result.error) {
      setError(result.error);
      setLoading(false);
      return;
      ...
      <div className="flex-1 px-6 flex flex-col justify-center max-w-sm mx-auto w-full pb-12">
        ...
        <form onSubmit={mode === 'login' ? handleLogin : handleSignup} className="space-y-4">
          {mode === 'signup' && (
            <>
              <div>
                <label className="text-[#B3B3B3] text-xs font-medium mb-2 block">Prénom</label>
              ...
              <div>
                <label className="text-[#B3B3B3] text-xs font-medium mb-2 block">Nom</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#808080]" />
                  <input
                    type="text"
              ...
              <div>
                <label className="text-[#B3B3B3] text-xs font-medium mb-2 block">Rôle</label>
                <select
                  value={role}
          ...
          <div>
            <label className="text-[#B3B3B3] text-xs font-medium mb-2 block">Email</label>
            <div className="relative">
          ...
          <div>
            <label className="text-[#B3B3B3] text-xs font-medium mb-2 block">Mot de passe</label>
            <div className="relative">
