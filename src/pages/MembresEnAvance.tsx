      ...
      <div className="max-w-lg mx-auto px-5 pb-32 pt-16">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-text-tertiary text-xs font-medium uppercase tracking-wider">Total en avance</span>
          </div>
          ...
          <p className="text-text-tertiary text-xs mt-1">
            {membresEnAvance.length} membre{membresEnAvance.length > 1 ? 's' : ''}
          </p>
        </div>
        ...
        {membresEnAvance.length === 0 ? (
          ...
          >
            ...
            <p className="text-text-tertiary text-xs mt-1">Tous les membres sont à jour</p>
          </div>
        ) : (
              ...
              >
                ...
                >
                  <span className="text-sm font-bold" style={{ color: '#FF6B00' }}>
                    {(membre.firstName || membre.first_name || '').charAt(0)}
                    {(membre.lastName || membre.last_name || '').charAt(0)}
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-text-primary text-sm font-semibold truncate">
                    {membre.firstName || membre.first_name} {membre.lastName || membre.last_name}
                  </p>
                  {membre.phone && (
                    <p className="text-text-tertiary text-xs">{membre.phone}</p>
                  )}
                </div>
