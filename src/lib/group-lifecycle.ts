...
import type { OrgUnit, Caisse, Group, Account, EventBudget, BudgetLine } from '@/types';
...
export function buildCreateEventBudget(
  ...
  const now = new Date().toISOString();
  const id = generateId();
  return { id, eventId, currency, revisedAt: null, revisedBy: null, createdAt: now };
}
...
export function applyRemoveBudgetLine(
  ...
  return budgetLines.filter(bl => !(bl.eventBudgetId === eventBudgetId && bl.id === lineId));
}
