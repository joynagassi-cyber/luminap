 ...
 */
export function cotisationCanModify(
  params: CotisationPolicyParams,
): PolicyResult {
  ...
  if (montantPaye === 0 && statut === "NON_PAYE") {
    if (isCulteVerrouille(culteDate)) {
      return {
        ok: false,
        rule: "COTISATION_CULTE_LOCKED",
        message: "Ce culte est verrouille (plus de 30 jours)",
      };
    }
    return { ok: true };
  }
  ...
  if (isCulteVerrouille(culteDate)) {
    return {
      ok: false,
      rule: "COTISATION_LOCKED",
      message: "Cotisation verrouillee — culte depasse 30 jours",
    };
  }
 ...
 */
export function cotisationValidateAmount(
  montantPayeCents: number,
): PolicyResult {
  if (montantPayeCents < 100) {
    return {
      ok: false,
      rule: "COTISATION_MIN_AMOUNT",
      message: "Le montant doit etre d'au moins 1 FCFA",
    };
  }
  if (montantPayeCents > 99999999900) {
    return {
      ok: false,
      rule: "COTISATION_MAX_AMOUNT",
      message: "Montant maximum depasse",
    };
  }
  return { ok: true };
}
...
export function transactionValidateAmount(amountCents: number): PolicyResult {
  if (amountCents <= 0) {
    return {
      ok: false,
      rule: "TRANSACTION_POSITIVE_AMOUNT",
  ...
  if (amountCents > 99999999900) {
    return {
      ok: false,
      rule: "TRANSACTION_MAX_AMOUNT",
      message: "Montant maximum atteint",
    };
  }
  return { ok: true };
}
...
export function transactionCanEdit(status: string): PolicyResult {
  if (status === "APPROVED") {
    return {
      ok: false,
      rule: "TRANSACTION_APPROVED_IMMUTABLE",
      message: "Une transaction approuvee ne peut pas etre modifiee",
    };
  }
  return { ok: true };
}
 ...
 */
export function versementCheckBalance(
  params: VersementPolicyParams,
): PolicyResult {
  ...
  if (amountCents <= 0) {
    return {
      ok: false,
      rule: "VERSEMENT_POSITIVE_AMOUNT",
      message: "Le montant doit etre superieur a 0",
    };
  }
  ...
  if (amountCents > balanceCents) {
    return {
      ok: false,
      rule: "VERSEMENT_INSUFFICIENT_BALANCE",
      message: "Solde insuffisant pour ce versement",
    };
  }
 ...
 */
export function versementValidateAtomic(
