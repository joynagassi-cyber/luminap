...
export class LifecycleService {
  ...
  async archive(entityType: ArchivableEntity, entityId: string, reason: string, actorId: string): Promise<void> {
    ...
    if (policy) {
      ...
      if (!check.ok) throw new Error(check.reason ?? `Cannot archive ${entityType}`);
    }
    ...
    // Read current state
    const result = await db.execute(`SELECT * FROM ${table} WHERE id = ?`, [entityId]);
    ...
    await auditLogRepo.write({
      ...
      entityId,
      beforeState: entity,
      afterState: { ...entity, status: newStatus, archivedAt, archivedBy, archiveReason, updatedAt: now },
      comment: reason,
    });
  ...
  async restore(entityType: ArchivableEntity, entityId: string, reason: string, actorId: string): Promise<void> {
    ...
    if (policy) {
      ...
      if (!check.ok) throw new Error(check.reason ?? `Cannot restore ${entityType}`);
    }
    ...
    if (!isEvent) {
      updates.push('archived_at = NULL', 'archived_by = NULL', 'archive_reason = NULL');
    }
    ...
    await auditLogRepo.write({
      ...
      entityId,
      beforeState: entity,
      afterState: { ...entity, status: newStatus, archivedAt: null, archivedBy: null, archiveReason: null, updatedAt: now },
      comment: reason,
    });
   ...
   */
  async isLifecycleActive(entityType: ArchivableEntity, entityId: string): Promise<boolean> {
    const table = ENTITY_TABLE[entityType];
    const db = getPowerSyncDatabase();
    const result = await db.execute(`SELECT status FROM ${table} WHERE id = ?`, [entityId]);
    const entity: any = result?.result?.[0];
