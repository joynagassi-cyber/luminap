...
import { NotificationCapability, type NotificationSendData } from '../notification';
...
function createMockPlugin() {
  const plugin = {
    ...
    Location: { requestPermission: vi.fn(), setShared: vi.fn(), isShared: vi.fn().mockResolvedValue(false) },
    Session: { addOutcome: vi.fn(), addUniqueOutcome: vi.fn(), addOutcomeWithValue: vi.fn() },
  };
...
describe('notification capability', () => {
  ...
  describe('requestPermission', () => {
    ...
    it('returns false when not initialized and permission fails', async () => {
      mockPlugin.Notifications.requestPermission.mockRejectedValue(new Error('not ready'));
      await notif.initialize();
      const result = await notif.requestPermission();
      expect(result).toBe(false);
    });
  ...
  describe('sendNotification', () => {
    ...
    it('records extra tags for targetRole', async () => {
      ...
      const callArgs = mockPlugin.setTags.mock.calls[0][0] as Record<string, string>;
    ...
    it('passes extraData as prefixed tags', async () => {
      await notif.initialize();
      await notif.sendNotification({ ...baseData, extraData: { event: 'meeting' } });
      const callArgs = mockPlugin.setTags.mock.calls[0][0] as Record<string, string>;
