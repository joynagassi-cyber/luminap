...
vi.mock('@/lib/orgContext', () => ({
  ...
  setOrganizationId: (id: string) => { _orgIdStore[0] = id; },
}));
...
vi.mock('@/store/useLocalStore', () => ({
  DEFAULT_USER: { role: 'TREASURIER' as const, id: 'mock-user-1', email: 'mock@example.com', firstName: 'Mock', lastName: 'User', org: { id: 'mock-org', name: 'Mock Org', type: 'Eglise' as const, accentColor: '#FF6B00' } },
...
const mockStorage = {
  ...
  setItem: vi.fn((key: string, value: string) => { _localStorage[key] = value; }),
  removeItem: vi.fn((key: string) => { delete _localStorage[key]; }),
  clear: vi.fn(() => { Object.keys(_localStorage).forEach(k => delete _localStorage[k]); }),
};
...
describe('e2e-auth: login with valid credentials', () => {
  it('signUpWithEmail validates email format and password length', async () => {
    ...
    authService['state'] = { session: null, user: null, profile: null, isLoading: false, error: null };
  ...
  it('signUpWithEmail stores session state when Supabase returns a user', async () => {
    ...
    const mockUser = { id: 'user-e2e-1', email: 'jean@example.com', user_metadata: { first_name: 'Jean', last_name: 'Dupont' } };
    const mockSession = { user: mockUser, access_token: 'mock-token', expires_at: Math.floor(Date.now() / 1000) + 7200 };

    // Simulate successful sign-up by directly setting state (mirrors what authService does internally)
    authService['setState']({ session: mockSession as any, user: mockUser, profile: { id: 'user-e2e-1', email: 'jean@example.com', role: 'TREASURIER' } as any });
  ...
  it('signUpWithEmail triggers selectRole and navigate in the full login flow', async () => {
    ...
    authService['setState']({
      ...
      profile: { id: 'user-flow-1', email: 'flow@example.com', role: 'TREASURIER', first_name: 'Flow', last_name: 'User' } as any,
    });
    ...
    expect(oneSignalService.login).toHaveBeenCalledWith(profile!.role as any, profile!.id);
  ...
  it('signInWithEmail validates credentials format before calling Supabase', async () => {
    ...
    const result = await authService.signInWithEmail('good@example.com', 'validpass123');
    expect(result).toBeDefined();
    expect(result.error).not.toBe('Please enter a valid email address.');
    expect(result.error).not.toBe('Password must be at least 8 characters long.');
  });
...
describe('e2e-auth: login with invalid credentials', () => {
  ...
  it('rejects password shorter than 8 characters during sign-in', async () => {
    ...
    const result = await authService.signInWithEmail('test@example.com', 'short');
  ...
  it('rejects empty password during sign-up', async () => {
    ...
    const result = await authService.signUpWithEmail('test@example.com', '', 'John', 'Doe', 'MEMBRE');
    expect(result.error).toBe('Password must be at least 8 characters long.');
  });
...
describe('e2e-auth: session persistence', () => {
  ...
  it('isValidSession checks session expiry correctly', async () => {
    ...
    expect(authService['isSessionExpiredOrExpiring'](futureSession as any)).toBe(false);
    ...
    expect(authService['isSessionExpiredOrExpiring'](expiredSession as any)).toBe(true);
    ...
    expect(authService['isSessionExpiredOrExpiring'](expiringSession as any)).toBe(true);
  });
  ...
  it('session with expires_at=0 is treated as expired', async () => {
    ...
    const session = { user: { id: 'user-exp-0' }, access_token: 'token', expires_at: 0 } as any;
  ...
  it('isSessionValid returns false for no session', async () => {
    ...
    expect(authService['isSessionExpiredOrExpiring']({ expires_at: 0 } as any)).toBe(true);
  });
  ...
  it('isSessionValid returns true for a valid session', async () => {
    ...
    expect(authService['isSessionExpiredOrExpiring'](validSession as any)).toBe(false);
  });
});
...
describe('e2e-auth: role-based redirects', () => {
  ...
  it('unauthenticated user on /dashboard is redirected to /auth', () => {
    const PROTECTED_ROUTES = [
      '/dashboard', '/finance', '/transaction', '/groups', '/events',
      '/versement', '/members', '/archives', '/reports', '/forms',
      '/custom-fields', '/report-builder', '/cotisations', '/saisie-rapide',
      '/culte', '/membres-en-avance', '/membre', '/trace', '/history',
...
describe('e2e-auth: logout flow', () => {
  it('signOut clears session, user, and profile state', async () => {
    ...
    authService['state'].profile = { id: 'user-logout-1', email: 'logout@test.com', role: 'TREASURIER' } as any;
...
describe('e2e-auth: full authentication journey', () => {
  it('sign up → set session → validate → role select → navigate to dashboard', async () => {
    ...
    authService['setState']({
      session: mockSession as any,
      user: mockUser,
      profile: { id: 'journey-user-1', email: 'journey@example.com', role: 'TREASURIER' } as any,
    });
  ...
  it('login → session valid → role redirect → then logout → redirect to auth', async () => {
    ...
    authService['state'].profile = { id: 'login-flow-user', email: 'login@example.com', role: 'MEMBRE' } as any;

    expect(authService.isAuthenticated()).toBe(true);
    // Test session expiry logic directly since isSessionValid calls real Supabase
    const sessionValid = authService['isSessionExpiredOrExpiring'](authService['state'].session);
  ...
  it('failed login attempt leaves user unauthenticated', async () => {
    ...
    const result = await authService.signInWithEmail('wrong@example.com', 'wrongpass');
