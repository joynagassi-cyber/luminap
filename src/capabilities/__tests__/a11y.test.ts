...
function readTsFile(relativePath: string): string {
  ...
  if (!fs.existsSync(abs)) throw new Error(`Missing component: ${relativePath}`);
 ...
 */
function extractAriaAttrs(source: string): Array<{ attr: string; value: string; line: number }> {
  const lines = source.split('\n');
  const results: Array<{ attr: string; value: string; line: number }> = [];
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Match aria-*="..." or aria-*={...}
    const ariaMatch = line.match(/(aria-[\w-]+)\s*=\s*(?:"([^"]*)"|(\{[^}]+\}))/g);
    if (ariaMatch) {
...
function hasFocusStyles(source: string): boolean {
  return /focus-visible:|focus:ring|focus-visible:outline|focus:outline-none|focus:ring-/.test(source);
}
...
function hasEscapeKeyHandler(source: string): boolean {
  return /e\.key\s*===\s*['"]Escape['"]/.test(source) || /key\s*===\s*['"]Escape['"]/.test(source);
}
...
describe('a11y — keyboard navigation', () => {
  it('ConfirmModal handles Escape key to close', () => {
    ...
    expect(hasKeyboardHandler(src), 'ConfirmModal should have a keyboard event listener').toBe(true);
    expect(hasEscapeKeyHandler(src), 'ConfirmModal should handle Escape key').toBe(true);
  });
  ...
  it('BottomNav has no keyboard trap — all items are focusable', () => {
    ...
    expect(src.includes('role="tab"'), 'BottomNav tabs should have role="tab"').toBe(true);
    // The nav landmark itself should have role="navigation"
    expect(src.includes('role="navigation"'), 'BottomNav should be a navigation landmark').toBe(true);
  });

  it('Sidebar has Escape key handler to dismiss', () => {
    const src = readUiFile('sidebar.tsx');
    expect(hasKeyboardHandler(src), 'Sidebar should listen for keyboard events').toBe(true);
  });
});
...
describe('a11y — aria labels', () => {
  it('ConfirmModal buttons have aria-label attributes', () => {
    ...
    expect(labelValues.some(l => l.includes('confirmLabel') || l.includes('Confirm'))).toBe(true);
  });

  it('TopHeader action buttons have aria-labels', () => {
    const src = readTsFile('TopHeader.tsx');
    const attrs = extractAriaAttrs(src);
    const labels = attrs.filter(a => a.attr.startsWith('aria-label')).map(a => a.value);
  ...
  it('BottomNav items all have aria-labels', () => {
    ...
    expect(labels.some(l => l.includes('Plus') || l.includes('options') || l.includes('More'))).toBe(true);
    expect(labels.some(l => l.includes('Fermer') || l.includes('Close') || l.includes('Fermer le menu'))).toBe(true);
    // FAB action
    expect(labels.some(l => l.includes('label') || l.includes('Transaction'))).toBe(true);
  });
  ...
  it('TopHeader notification button declares dialog role', () => {
    const src = readTsFile('TopHeader.tsx');
    expect(src.includes('aria-haspopup="dialog"'), 'Notification button should declare aria-haspopup="dialog"').toBe(true);
  });
});
...
describe('a11y — role attributes', () => {
  it('BottomNav uses correct ARIA roles for tab pattern', () => {
    const src = readTsFile('BottomNav.tsx');
    expect(src.includes('role="tablist"'), 'Tab list container should have role="tablist"').toBe(true);
    expect(src.includes('role="tab"'), 'Each tab should have role="tab"').toBe(true);
    expect(src.includes('aria-selected'), 'Tabs should expose aria-selected state').toBe(true);
  });

  it('Alert component uses role="alert"', () => {
    const src = readUiFile('alert.tsx');
    expect(src.includes('role="alert"'), 'Alert should use role="alert"').toBe(true);
  });

  it('Carousel uses landmark roles', () => {
    const src = readUiFile('carousel.tsx');
    expect(src.includes('role="region"'), 'Carousel should be a region').toBe(true);
    expect(src.includes('role="group"'), 'Slide items should be in a group').toBe(true);
    expect(src.includes('aria-roledescription="carousel"'), 'Carousel should declare roledescription').toBe(true);
    expect(src.includes('aria-roledescription="slide"'), 'Slides should declare roledescription').toBe(true);
  });
  ...
  it('Breadcrumb uses proper navigation and link roles', () => {
    ...
    expect(src.includes('aria-label="breadcrumb"') || src.includes("aria-label='breadcrumb'"), 'Breadcrumb should have aria-label="breadcrumb"').toBe(true);
    // Check for link roles or native <a> elements
    expect(src.includes('role="link"') || src.includes('<a ') || src.includes('href='), 'Breadcrumb links should have role="link" or href').toBe(true);
    expect(src.includes('aria-current="page"'), 'Active breadcrumb should have aria-current="page"').toBe(true);
  });

  it('Pagination uses landmark and aria-current roles', () => {
    const src = readUiFile('pagination.tsx');
    expect(src.includes('role="navigation"'), 'Pagination should be a navigation landmark').toBe(true);
    expect(src.includes('aria-label="pagination"'), 'Pagination should have aria-label="pagination"').toBe(true);
    expect(src.includes('aria-current'), 'Active page should declare aria-current').toBe(true);
  });

  it('Dialog close button has sr-only label', () => {
    const src = readUiFile('dialog.tsx');
    expect(src.includes('<span className="sr-only">Close</span>'), 'Dialog close should have sr-only label').toBe(true);
  });
});
...
describe('a11y — focus management', () => {
  it('Button component includes focus-visible ring styles', () => {
    ...
    expect(hasFocusStyles(src), 'Button should have focus-visible styles').toBe(true);
  });

  it('Input component includes focus-visible ring styles', () => {
    const src = readUiFile('input.tsx');
    expect(hasFocusStyles(src), 'Input should have focus-visible styles').toBe(true);
  });

  it('Checkbox includes focus-visible ring styles', () => {
    const src = readUiFile('checkbox.tsx');
    expect(hasFocusStyles(src), 'Checkbox should have focus-visible styles').toBe(true);
  });
  ...
  it('AlertDialog close button is focusable with visible ring', () => {
    ...
    expect(src.includes('aria-label') || src.includes('button') || src.includes('Button'), 'AlertDialog close should manage focus').toBe(true);
  });

  it('Sidebar toggle has aria-label for screen readers', () => {
    const src = readUiFile('sidebar.tsx');
    expect(src.includes('aria-label="Toggle Sidebar"') || src.includes('aria-label=\'Toggle Sidebar\''), 'Sidebar toggle should be labelled').toBe(true);
    expect(hasSROnly(src), 'Sidebar toggle should use sr-only for icon-only button').toBe(true);
  });

  it('Disabled elements have pointer-events-none to prevent focus', () => {
    const src = readUiFile('button.tsx');
    expect(src.includes('disabled:pointer-events-none') || src.includes('pointer-events-none'), 'Disabled buttons should not receive focus').toBe(true);
  });
});
...
describe('a11y — sr-only text', () => {
  it('Carousel previous/next buttons have sr-only labels', () => {
    const src = readUiFile('carousel.tsx');
    expect(src.includes('<span className="sr-only">Previous slide</span>'), 'Prev button should have sr-only label').toBe(true);
    expect(src.includes('<span className="sr-only">Next slide</span>'), 'Next button should have sr-only label').toBe(true);
  });

  it('Breadcrumb "More" item has sr-only label', () => {
    const src = readUiFile('breadcrumb.tsx');
    expect(src.includes('<span className="sr-only">More</span>'), 'Breadcrumb more should have sr-only label').toBe(true);
  });

  it('Pagination "More pages" has sr-only label', () => {
    const src = readUiFile('pagination.tsx');
    expect(src.includes('<span className="sr-only">More pages</span>'), 'Pagination more should have sr-only label').toBe(true);
  });
});
...
describe('a11y — aria-hidden correctness', () => {
  it('Decorative elements use aria-hidden="true"', () => {
    ...
    expect(src.includes('aria-hidden="true"'), 'Decorative modal handle should be aria-hidden').toBe(true);
  });

  it('Navigation menu hidden items use aria-hidden', () => {
    const src = readUiFile('navigation-menu.tsx');
    expect(src.includes('aria-hidden="true"'), 'Hidden nav items should be aria-hidden').toBe(true);
  });
});
...
describe('a11y — aria-expanded', () => {
  it('BottomNav more-button exposes aria-expanded state', () => {
    const src = readTsFile('BottomNav.tsx');
    expect(src.includes('aria-expanded'), 'More button should expose aria-expanded').toBe(true);
  });
  ...
  it('Dropdown menu items are keyboard accessible (outline-none + focus styles)', () => {
    ...
    expect(hasFocusStyles(src) || src.includes('focus:') || src.includes('focus-bg'), 'Dropdown menu items should have focus styles').toBe(true);
  });
});
