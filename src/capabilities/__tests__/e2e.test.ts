...
vi.mock("@/lib/orgContext", () => ({
  ...
  setOrganizationId: (id: string) => {
    _orgIdStore[_orgIdStore.length - 1] = id;
  },
}));
...
function psExecute(sql: string, params: any[] = []): { array: any[]; result: any[] } {
    ...
    async list(filters: any = {}) {
      ...
      if (filters.entityType)
        entries = entries.filter(
          (e: any) => e.entityType === filters.entityType,
        );
...
describe("e2e: group membership → relationship", () => {
  it("add member → isMember returns true", async () => {
    const memId = await relationship.addMembership(
      "group-a",
      "user-1",
      "MEMBRE",
      "actor-1",
  ...
  it("isMember returns false for non-existent membership", async () => {
    expect(await relationship.isMember("group-missing", "user-missing")).toBe(
      false,
    );
  });
...
describe("e2e: archive → restore → audit trail", () => {
  ...
  it("isLifecycleActive returns true for ACTIVE entity", async () => {
    ...
    expect(await lifecycle.isLifecycleActive("Group", "grp-active-1")).toBe(
      true,
    );
  });
  ...
  it("isLifecycleActive returns false for ARCHIVED entity", async () => {
    ...
    expect(await lifecycle.isLifecycleActive("Group", "grp-archived-1")).toBe(
      false,
    );
  });
  ...
  it("archive throws when entity not found", async () => {
    lifecycle.register("Group", { canArchive: async () => ({ ok: true }) });
    await expect(
      lifecycle.archive("Group", "nonexistent", "reason", "actor-1"),
  ...
  it("archive throws when policy rejects", async () => {
    ...
    await expect(
      lifecycle.archive("Group", "grp-block-1", "reason", "actor-1"),
...
describe("e2e: cross-capability integration", () => {
  ...
  it("security capability: user role gates transaction approval", () => {
    ...
    expect(
      security.hasPermission("PASTEUR_PRINCIPAL", "transaction:approve"),
    ).toBe(true);
