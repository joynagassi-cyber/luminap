...
describe('policy capability', () => {
  ...
  describe('cotisationCanModify', () => {
    it('allows modification of NON_PAYE cotisation within 30 days', () => {
      const result = cotisationCanModify({
        culteDate: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        statut: 'NON_PAYE',
        montantPaye: 0,
    ...
    it('blocks modification of NON_PAYE cotisation after 30-day lock', () => {
      const result = cotisationCanModify({
        culteDate: new Date(Date.now() - 35 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        statut: 'NON_PAYE',
        montantPaye: 0,
        montantObligatoire: 5000,
      });
  ...
  describe('versementCheckBalance', () => {
    it('allows versement when amount equals balance', () => {
      const result = versementCheckBalance({ balanceCents: 500000, amountCents: 500000 });
      expect(result.ok).toBe(true);
    });

    it('allows versement when amount is below balance', () => {
      const result = versementCheckBalance({ balanceCents: 500000, amountCents: 300000 });
      expect(result.ok).toBe(true);
    });

    it('blocks versement when amount exceeds balance', () => {
      const result = versementCheckBalance({ balanceCents: 500000, amountCents: 600000 });
      expect(result.ok).toBe(false);
      expect(result.rule).toBe('VERSEMENT_INSUFFICIENT_BALANCE');
    });

    it('blocks versement with zero amount', () => {
      const result = versementCheckBalance({ balanceCents: 500000, amountCents: 0 });
      expect(result.ok).toBe(false);
      expect(result.rule).toBe('VERSEMENT_POSITIVE_AMOUNT');
    });

    it('blocks versement with negative amount', () => {
      const result = versementCheckBalance({ balanceCents: 500000, amountCents: -100 });
      expect(result.ok).toBe(false);
      expect(result.rule).toBe('VERSEMENT_POSITIVE_AMOUNT');
    });

    it('allows versement when balance is zero and amount is zero — no, amount must be positive', () => {
      const result = versementCheckBalance({ balanceCents: 0, amountCents: 100 });
      expect(result.ok).toBe(false);
      expect(result.rule).toBe('VERSEMENT_INSUFFICIENT_BALANCE');
    });
  });
  ...
  describe('PolicyService singleton', () => {
    ...
    it('versement.checkBalance delegates correctly', () => {
      expect(policy.versement.checkBalance({ balanceCents: 100000, amountCents: 50000 }).ok).toBe(true);
      expect(policy.versement.checkBalance({ balanceCents: 100000, amountCents: 200000 }).ok).toBe(false);
    });
  });
});
