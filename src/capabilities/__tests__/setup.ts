  ...
  return {
    execute: async (sql: string, params: any[] = []) => {
      ...
      if (match(/^SELECT \* FROM (\w+) WHERE id = \?$/)) {
        ...
        return { result: (mockRows[table] ?? []).find((r: any) => r.id === params[0]) ? [(mockRows[table] ?? []).find((r: any) => r.id === params[0])] : [] };
      }
      ...
      if (match(/^SELECT \* FROM (\w+) WHERE org_id = \? AND status = \?/)) {
        ...
        return { result: (mockRows[table] ?? []).filter((r: any) => r.status === status) };
      }
      ...
      if (match(/^INSERT/)) {
        ...
        if (table) {
          ...
          const keys = sql.match(/INSERT INTO \w+ \(([^)]+)\)/)?.[1]?.split(',').map((k: string) => k.trim()) ?? [];
      ...
      if (match(/^DELETE FROM (\w+) WHERE id = \?$/)) {
        ...
        if (mockRows[table]) {
          mockRows[table] = mockRows[table].filter((r: any) => r.id !== params[0]);
        }
        return { result: [] };
      }
      ...
      if (match(/^UPDATE/)) {
        ...
        if (mockRows[table]) {
          ...
          if (idx !== -1) {
            ...
            if (setMatch) {
              const setClauses = setMatch[1].split(',').map((c: string) => c.trim());
              let paramIdx = 0;
              setClauses.forEach((clause: string) => {
                const eqIdx = clause.indexOf('=');
                const col = eqIdx !== -1 ? clause.substring(0, eqIdx).trim() : clause.split(/\s+/)[0];
                const valPart = eqIdx !== -1 ? clause.substring(eqIdx + 1).trim() : '';
                if (col && col !== 'id') {
