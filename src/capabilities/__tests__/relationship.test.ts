...
describe("relationship capability", () => {
  ...
  describe("addMembership", () => {
    ...
    it("generates a unique id for each call", async () => {
      const id1 = await relationship.addMembership(
        "group-1",
        "member-1",
        "MEMBRE",
        "actor-1",
      );
      const id2 = await relationship.addMembership(
        "group-1",
        "member-2",
        "MEMBRE",
        "actor-1",
  ...
  describe("removeMembership", () => {
    ...
    it("is a no-op for a non-existent membership id", async () => {
      // Should not throw
      await expect(
        relationship.removeMembership("nonexistent", "actor-1"),
