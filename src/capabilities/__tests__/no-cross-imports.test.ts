...
describe('Capability Architecture Boundaries', () => {
  const capabilities = ['workflow', 'lifecycle', 'relationship', 'resource', 'security'];
  ...
  for (const cap of capabilities) {
    ...
    it(`'${cap}' must not import from another capability`, () => {
      for (const otherCap of capabilities) {
        if (otherCap !== cap) {
          expect(content).not.toContain(`@/capabilities/${otherCap}`,
  ...
  it('should have no circular capability dependencies', () => {
    ...
    for (const cap of capabilities) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.startsWith('import ') && trimmed.includes("@/capabilities/")) {
          allImports.push(trimmed);
        }
      }
    }
    // No cross-capability imports means no possible cycles
    expect(allImports, `Cross-capability imports found: ${allImports.join(', ')}`).toHaveLength(0);
  });
});
