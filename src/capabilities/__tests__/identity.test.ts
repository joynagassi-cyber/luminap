...
import { IdentityService, type IdentityProfile, type IdentityProfileUpdate } from '../identity';
...
describe('identity capability', () => {
  ...
  describe('getProfile', () => {
    ...
    it('returns a profile with metadata', () => {
      identity.createProfile('user-1', 'alice@example.com', 'Alice', { role: 'admin' });
      const profile = identity.getProfile('user-1');
  ...
  describe('createProfile', () => {
    ...
    it('returns the existing profile when called again with the same id', () => {
      const first = identity.createProfile('user-1', 'alice@example.com', 'Alice');
      const second = identity.createProfile('user-1', 'changed@example.com', 'Changed');
    ...
    it('accepts optional metadata', () => {
      const profile = identity.createProfile('user-1', 'alice@example.com', 'Alice', { role: 'admin', tier: 'premium' });
  ...
  describe('updateProfile', () => {
    ...
    it('updates email', () => {
      identity.createProfile('user-1', 'alice@example.com', 'Alice');
      const updated = identity.updateProfile('user-1', { email: 'newalice@example.com' });
      expect(updated).not.toBeNull();
    ...
    it('updates metadata', () => {
      identity.createProfile('user-1', 'alice@example.com', 'Alice', { role: 'admin' });
      const updated = identity.updateProfile('user-1', { metadata: { role: 'viewer' } });
      expect(updated!.metadata).toEqual({ role: 'viewer' });
    });

    it('returns null for a non-existent profile', () => {
      const result = identity.updateProfile('nonexistent', { displayName: 'Nobody' });
      expect(result).toBeNull();
    });

    it('preserves existing fields when updating only one field', () => {
      identity.createProfile('user-1', 'alice@example.com', 'Alice', { role: 'admin' });
      const updated = identity.updateProfile('user-1', { displayName: 'Alice Updated' });
    ...
    it('merges metadata rather than replacing — no, it replaces per spec', () => {
      identity.createProfile('user-1', 'alice@example.com', 'Alice', { role: 'admin' });
      const updated = identity.updateProfile('user-1', { metadata: { tier: 'premium' } });
  ...
  describe('deleteProfile', () => {
    ...
    it('allows re-creation after deletion', () => {
      ...
      const fresh = identity.createProfile('user-1', 'alice@example.com', 'Alice');
