...
export function setupLuminaTheme(): void {
  ...
  root.style.setProperty('--ion-tab-bar-color-selected', LUMINA_TOKENS.accentPrimary);
  ...
  root.style.setProperty('--ion-safe-area-left', 'env(safe-area-inset-left, 0px)');
  root.style.setProperty('--ion-safe-area-right', 'env(safe-area-inset-right, 0px)');

  // Typography
  root.style.setProperty('--ion-font-family', '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif');
