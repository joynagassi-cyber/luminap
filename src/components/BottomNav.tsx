...
import { Home, TrendingUp, Building2, Settings, MoreHorizontal, Calendar, Wallet } from 'lucide-react';
...
const EXTRA_FEATURES: { path: string; label: string; icon: typeof Calendar; color: string }[] = [
  ...
  { path: '/versement', label: 'Versement', icon: Wallet, color: '#1DB954' },
];
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="flex flex-col items-center justify-center gap-1 px-4 py-2 active:scale-95 transition-transform"
        >
          <MoreHorizontal className="w-5 h-5" style={{ color: ['/events', '/versement'].includes(location.pathname) ? '#FF6B00' : '#808080' }} />
          <span className="text-xs font-medium" style={{ color: ['/events', '/versement'].includes(location.pathname) ? '#FF6B00' : '#808080' }}>Plus</span>
        </button>
      </nav>

      {/* Floating "+" button — opens caisse quick action */}
      <button
        onClick={() => setDrawerOpen(true)}
