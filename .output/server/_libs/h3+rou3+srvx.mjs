...
function wrapFetch(server) {
	...
	return middleware.length === 0 ? fetchHandler : (request) => callMiddleware$2(request, fetchHandler, middleware, 0);
}
function callMiddleware$2(request, fetchHandler, middleware, index) {
	if (index === middleware.length) return fetchHandler(request);
	return middleware[index](request, () => callMiddleware$2(request, fetchHandler, middleware, index + 1));
}
...
var H3Event$1 = class {
	...
	constructor(req, context, app) {
		...
		this.app = app;
		const _url = req._url;
		const url = _url && _url instanceof URL ? _url : new FastURL$1(req.url);
		if (url.pathname.includes("%")) url.pathname = decodePathname$1(url.pathname);
		this.url = url;
	}
	get res() {
...
function toResponse$1(val, event, config = {}) {
	if (typeof val?.then === "function") return val.then((resolvedVal) => toResponse$1(resolvedVal, event, config), (r) => toResponse$1(typeof r === "number" ? new HTTPError$1({ status: r }) : r, event, config));
...
function prepareResponse$1(val, event, config, nested) {
	...
	if (val && val instanceof Error) {
		...
		return onError && !nested ? Promise.resolve(onError(error, event)).catch((error) => error).then((newVal) => prepareResponse$1(newVal ?? val, event, config, true)) : errorResponse$1(error, config.debug, errHeaders);
	}
...
function callMiddleware$1(event, middleware, handler, index = 0) {
	...
	return isUnhandledResponse$1(ret) ? next() : typeof ret?.then === "function" ? ret.then((resolved) => isUnhandledResponse$1(resolved) ? next() : resolved) : ret;
}
function isUnhandledResponse$1(val) {
...
function defineLazyEventHandler(loader) {
	let handler;
	let promise;
	return defineHandler$1(function lazyHandler(event) {
...
function parseURLEncodedBody(body) {
	...
	const parsedForm = new NullProtoObj$1();
...
function endIndex$1(str, min, len) {
	const index = str.indexOf(";", min);
	return index === -1 ? len : index;
}
function eqIndex$1(str, min, max) {
	const index = str.indexOf("=", min);
	return index < max ? index : -1;
}
function valueSlice$1(str, min, max) {
	if (min === max) return "";
	let start = min;
	let end = max;
	do {
...
function parse$1(str, options) {
	...
	let index = 0;
	do {
		const eqIdx = eqIndex$1(str, index, len);
		if (eqIdx === -1) break;
		const endIdx = endIndex$1(str, index, len);
		if (eqIdx > endIdx) {
			index = str.lastIndexOf(";", eqIdx - 1) + 1;
			continue;
		}
		const key = valueSlice$1(str, index, eqIdx);
		if (options?.filter && !options.filter(key)) {
			index = endIdx + 1;
			continue;
		}
		const val = dec(valueSlice$1(str, eqIdx + 1, endIdx));
		if (allowMultiple) {
...
function createError$1(arg1, arg2) {
	return new HTTPError$1(arg1, arg2);
}
//#endregion
...
//#endregion
export { setCookie as _, setHeader as a, serve as b, HTTPResponse$1 as c, defineLazyEventHandler as d, deleteCookie as f, readBody as g, getRouterParam as h, getHeader as i, createError$1 as l, getQuery as m, defineEventHandler as n, H3Core as o, getCookie$1 as p, getCookie as r, HTTPError$1 as s, createError as t, defineHandler$1 as u, toEventHandler as v, NodeResponse$1 as y };
